...
- (void) peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error;
{
	...
	for (characteristic in characteristics) {
        ...
        if ([[characteristic UUID] isEqual:BLE_HANDLE_TEST_READ_UUID]) {
            NSLog(@"Discovered My Characteristic"); BLE_HANDLE_TEST_READ_CHARACTERISTIC = [characteristic retain];
			[peripheral readValueForCharacteristic:characteristic];
		}
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_TEST_WRITE_UUID])BLE_HANDLE_TEST_WRITE_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_ALARM_0_UUID])BLE_HANDLE_ALARM_0_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_ALARM_1_UUID])BLE_HANDLE_ALARM_1_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_ALARM_2_UUID])BLE_HANDLE_ALARM_2_CHARACTERISTIC = [characteristic retain];
        ...
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_SPORTS_DESC_UUID])BLE_HANDLE_SPORTS_DESC_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_DEVICE_ID_UUID])BLE_HANDLE_DEVICE_ID_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_FILE_DESC_UUID])BLE_HANDLE_FILE_DESC_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_GPS_INFO_UUID])BLE_HANDLE_GPS_INFO_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_GESTURE_UUID])BLE_HANDLE_CONF_GESTURE_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_WORLDCLOCK_0_UUID])BLE_HANDLE_CONF_WORLDCLOCK_0_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_WORLDCLOCK_1_UUID])BLE_HANDLE_CONF_WORLDCLOCK_1_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_WORLDCLOCK_2_UUID])BLE_HANDLE_CONF_WORLDCLOCK_2_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_WORLDCLOCK_3_UUID])BLE_HANDLE_CONF_WORLDCLOCK_3_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_WORLDCLOCK_4_UUID])BLE_HANDLE_CONF_WORLDCLOCK_4_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_WORLDCLOCK_5_UUID])BLE_HANDLE_CONF_WORLDCLOCK_5_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_WATCHFACE_UUID])BLE_HANDLE_CONF_WATCHFACE_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_GOALS_UUID])BLE_HANDLE_CONF_GOALS_CHARACTERISTIC = [characteristic retain];
        else if ([[characteristic UUID] isEqual:BLE_HANDLE_CONF_USER_PROFILE_UUID])BLE_HANDLE_CONF_USER_PROFILE_CHARACTERISTIC = [characteristic retain];
...
- (void) peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    //[KreyosBluetoothController sharedInstance].currentlyDisplayingService = [[KreyosBluetoothController sharedInstance] serviceForPeripheral:peripheral];
...
- (void) peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if ([characteristic.UUID isEqual:BLE_HANDLE_TEST_WRITE_UUID] )
    {
        //[periphe ralDelegate alarmServiceDidChangeValueForCharacterristic:self];
    }
#endif
...
{
    [self writeData:[NSData dataWithBytes:value length:sizeof(int8_t)] characterisc:BLE_HANDLE_TEST_WRITE];
}
...
-(void) writeGpsInfo:(int16_t)v1 value2:(int16_t)v2 value3:(int16_t)v3 value4:(int16_t)v4
{
    int16_t data16[4];
    data16[0] = v1;
    data16[1] = v2;
